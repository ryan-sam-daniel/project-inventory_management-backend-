...
public class VendorDaoImpl implements VendorDao<Vendor> {
    ...
    public VendorDaoImpl(final DataSource dataSource) {
        this.dataSource = dataSource;
    }

    @Override
    public boolean checkExistence(final String phoneNo){
        final String query = "Select id, name, phone_no, city, pincode from \"inventoryDB\".\"Vendor\" where phone_no = ?";
    ...
    public int store(final Vendor vendor,final Connection connection){
        final boolean isAvailable = checkExistence(vendor.getPhoneNo());

        if (isAvailable) {
            final Vendor existingVendor = get(vendor.getPhoneNo());
            return existingVendor.getId();
        }

        final String query = "INSERT INTO \"inventoryDB\".\"Vendor\"(name, phone_no, city, pincode) VALUES (?, ?, ?, ?)";

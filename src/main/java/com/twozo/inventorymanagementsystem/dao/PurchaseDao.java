...
@Repository
@Profile("dev")
public class PurchaseDao implements TradeDao<Vendor, Purchase, PurchaseItem> {
    private final static Logger logger = LoggerFactory.getLogger(PurchaseDao.class);
    private final VendorDao<Vendor> vendorDAO;
    private final PaymentDao<Payment> paymentDao;
    private final DataSource dataSource;

    public PurchaseDao(final VendorDao<Vendor> vendorDAO, final PaymentDao<Payment> paymentDao, final DataSource dataSource){
    ...
    public int store(final Purchase purchase, final Connection connection){
        final String query = "Insert into \"inventoryDB\".\"Purchase_Order\" (vendor_id, transaction_id, purchase_date, sub_total, final_amount) values (?, ?, ?, ?, ?)";
    ...
    public boolean storeItem(final List<PurchaseItem> cart, final int purchaseId, final Connection connection){
        ...
        for (final PurchaseItem purchase : cart){
            final String query = "Insert into \"inventoryDB\".\"Purchase_Item\" (purchase_order_id,product_id,quantity,selling_price,sub_total,tax_amount,final_amount) values (?, ?, ?, ?, ?, ?, ?)";
